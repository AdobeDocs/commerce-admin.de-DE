...
- In der Importdatei wird ein nicht vorhandener Dienst oder ein komplexer Datenname angegeben, z. B. eine Spalte mit einer `_<non-existing name>` -Überschrift.

## Importvorgänge

| Vorgang | Beschreibung |
| --------- | ----------- |
| Hinzufügen/Aktualisieren | Zu den vorhandenen Produktdaten für die vorhandenen Einträge in der Datenbank werden neue Produktdaten hinzugefügt. Alle Felder außer `sku` kann aktualisiert werden.<br><br>Neue, in den Importdaten angegebene Steuerklassen werden automatisch erstellt.<br><br>Neue Produktkategorien, die in der Importdatei angegeben sind, werden automatisch erstellt.<br><br>In der Importdatei angegebene neue SKUs werden automatisch erstellt.<br><br>**_Hinweis:_**Bei Produkten können Sie alle Felder außer SKU durch Importieren aktualisieren.<br><br>**_Wichtig:_** Mehrere Feldwerte wie Websites oder Kategorien können nicht mit der _Hinzufügen/Aktualisieren_ Importverhalten. Diese Felder bleiben nach dem Import in der Datenbank, wenn sie nicht in der CSV-Datei aufgeführt sind. |
...
1. under _Importeinstellungen_, set **[!UICONTROL Entity Type]** auf einen der folgenden Werte zu:
...
1. Klicken **[!UICONTROL Download Sample File]**.
...
1. Vergewissern Sie sich, dass die Größe Ihrer Importdatei die in der Nachricht angegebene Grenze nicht überschreitet.
   ...
   ![Benachrichtigung zur Größe des Datenimports](./assets/data-import-size-notification.png){width="600"}
...
1. Satz **[!UICONTROL Import Behavior]** auf einen der folgenden Werte zu:

   - `Add/Update` (Bei Produkten können Sie alle Felder außer SKU durch Importieren aktualisieren.)
   - `Replace`
   - `Delete`
...
1. Für **[!UICONTROL Allowed Errors Count]** die Anzahl der Fehler angeben, die auftreten können, bevor der Import abgebrochen wird.

   Der Standardwert ist 10.

1. Den Standardwert eines Kommas akzeptieren (`,`) für **[!UICONTROL Field separator]**.

1. Den Standardwert eines Kommas akzeptieren (`,`) für **[!UICONTROL Multiple value separator]**.
...
1. Standardwert akzeptieren `_EMPTY_VALUE_` für **[!UICONTROL Empty attribute value constant]**.
...
1. Klicks **[!UICONTROL Choose File]** , um die zu importierende Datei auszuwählen.

1. Suchen Sie die CSV-Datei, die Sie für den Import vorbereitet haben, und klicken Sie auf **[!UICONTROL Open]**.

1. Für **[!UICONTROL Images File Directory]** Geben Sie den relativen Pfad zum Speicherort auf dem Commerce-Server ein, auf dem die hochgeladenen Bilder gespeichert sind.

   Beispiel: `product_images`.

   >[!NOTE]
   >
   >Einstieg in Adobe Commerce und Magento Open Source `2.3.2` -Version, der Pfad, der in _[!UICONTROL Images File Directory]_Verkettet zum Importieren in den Ordner &quot;images base&quot;: `<Magento-root-folder>/var/import/images`. Platzieren Sie beispielsweise die `product_images` -Dateien in `<Magento-root-directory>/var/import/images/product_images` Ordner. Der Basisordner für Importbilder kann im `\Magento\ImportExport\etc\config.xml` -Datei. Wenn das Remote-Speichermodul aktiviert ist, importieren Sie Dateien in die `<remote-storage-root-directory>/var/import/images/product_images` Ordner.
...
Commerce verwaltet einen Datensatz, der in Ihren Store importiert wurde, einschließlich Startdatum und -zeit, Benutzer, Ausführungszeit und Link zur importierten Datei. Die _Ausführungszeit_ ist die Dauer des Importvorgangs.

**_Anzeigen des Importverlaufs:_**

Im _Admin_ Seitenleiste, navigieren Sie zu **[!UICONTROL System]** > _[!UICONTROL Data Transfer]_>**[!UICONTROL Import History]**.

![Datenimport-Verlauf](./assets/data-import-history.png){width="600" zoomable="yes"}

>[!NOTE]
>
>Standardmäßig befinden sich die Importverlauf-Dateien im `<Magento-root-directory>/var/import_history` Ordner. Wenn das Remote-Speichermodul aktiviert ist, befinden sich die Importverlauf-Dateien im `<remote-storage-root-directory>/import_export/import_history` Ordner.
...
So laden Sie die _Importiert/Fehler_ Datei, klicken Sie auf **[!UICONTROL Download]**.
