...
1. Gehen Sie im Abschnitt **[!UICONTROL Attribute Properties]** wie folgt vor:
   ...
   - Geben Sie eine **[!UICONTROL Attribute Code]** ein, die das Attribut innerhalb des Systems identifiziert.
     ...
     >[!TIP]
     >
     >**_Shortcut:_** Um nur die erforderlichen Felder auszufüllen, scrollen Sie nach unten zu [!UICONTROL Storefront Properties], geben Sie den [!UICONTROL Sort Order] ein und speichern Sie ihn.
...
1. Um die Genauigkeit der in das Feld eingegebenen Daten zu überprüfen, bevor der Datensatz gespeichert wird, setzen Sie **[!UICONTROL Input Validation]** auf den Typ der Daten, die in dem Feld zulässig sein sollen. Die verfügbaren Werte hängen von der angegebenen _[!UICONTROL Input Type]_ab.
   ...
   >[!NOTE]
   >
   >Für systemdefinierte Kundenadressattribute wie _Telefon_, _Stadt_ und _Straße_ erzwingt Commerce die integrierte Server-seitige Validierung, unabhängig von der **[!UICONTROL Input Validation]**. Diese Standardregeln beschränken die zulässigen Zeichen für jedes Feld (Telefonnummern dürfen beispielsweise nur Ziffern, Leerzeichen und bestimmte Symbole enthalten). Die **[!UICONTROL Input Validation]**-Einstellung fügt weitere Einschränkungen hinzu, kann jedoch die integrierte Validierung nicht überschreiben, die nicht über die Admin-Benutzeroberfläche deaktiviert werden kann.
...
| [!UICONTROL Input Validation] | Die Auswahl der Optionen wird durch den Eingabetyp bestimmt. Optionen: <br/>**`None`**: Das Feld verfügt während der Dateneingabe über keine Eingabevalidierung.<br/>**`Alphanumeric`** - Akzeptiert eine beliebige Kombination von Zahlen (0-9) und alphabetischen Zeichen (a-z, A-Z) bei der Dateneingabe. <br/>**`Alphanumeric with Space`**- Ermöglicht Platzierungen in der Straße Adresse, um die Anforderungen an die maximale Länge des Trägers zu erfüllen. Während des Checkouts können Kunden Buchstaben (a-z, A-Z), Zahlen (0-9) und Leerzeichen eingeben. Beim Speichern der Adresse werden zusätzliche Leerzeichen abgeschnitten.<br/>**`Numeric Only`** - Akzeptiert bei der Dateneingabe nur Zahlen (0-9). <br/>**`Alpha Only`**- Akzeptiert bei der Dateneingabe nur alphabetische Zeichen (a-z, A-Z).<br/>** URL **- Akzeptiert bei der Dateneingabe nur eine URL.<br/>**`Email`** - Akzeptiert bei der Dateneingabe nur eine E-Mail-Adresse. <br/>**`Length Only`**: Überprüft die Eingabe anhand der Länge der in das Feld eingegebenen Daten.<br/><br/>** Hinweis:**Für systemdefinierte Attribute wie _Telefon_, _Stadt_ und _Straße_ wird zusätzlich zu **[!UICONTROL Input Validation]**Einstellungen immer eine integrierte Server-seitige Validierung erzwungen. Diese Standardregeln beschränken die zulässigen Zeichen für jedes Feld und können nicht überschrieben werden. Die **[!UICONTROL Input Validation]**-Einstellung fügt nur weitere Einschränkungen hinzu. |
...
| [!UICONTROL Grid Filter Condition Type] | Gibt Filterzuordnungsbedingungen für Attribute in Suchvorgängen aus dem Raster an. Das _[!UICONTROL Search by keyword]_Feld für das Raster ist davon nicht betroffen. Optionen: `Partial Match` / `Prefix Match` / `Full Match` |
