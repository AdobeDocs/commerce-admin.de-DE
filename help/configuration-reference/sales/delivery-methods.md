...
![Pauschalsatz](./assets/delivery-methods-flat-rate.png)<!-- zoom -->

<!-- [Flat Rate](https://docs.magento.com/user-guide/shipping/shipping-flat-rate.html) -->

| Feld | [Anwendungsbereich](../../getting-started/websites-stores-views.md#scope-settings) | Beschreibung |
|--- |--- |--- |
| [!UICONTROL Enabled] | Webseite | Wenn diese Option aktiviert ist, wird die Pauschalrate als Option im _Geschätzte Schifffahrt und Steuer_ im Warenkorb und im _Versand_ während des Checkouts. Optionen: `Yes` / `No` |
...
| [!UICONTROL Type] | Webseite | Beschreibt die Art der Berechnung, die zur Bestimmung des Pauschalsatzes verwendet wird. Optionen: <br/>**`None`**- Es wird keine Berechnung verwendet. Legt den Pauschalsatz auf null fest, was dem kostenlosen Versand entspricht.<br/>**`Per Order`** - berechnet einen Pauschalpreis für die gesamte Bestellung. <br/>**`Per Item`**- berechnet für jeden Artikel im Warenkorb einen gesonderten Pauschalsatz. Der Prozentsatz wird mit der Anzahl der Artikel im Warenkorb multipliziert, auch wenn die Gesamtmenge eine Kombination verschiedener Artikel enthält. |
...
| Steuern auf Betrag einschließen | Webseite | Bestimmt, ob die Steuer in der Berechnung des Mindestauftragsbetrags enthalten ist. Optionen: <br/>**Ja** - Bei der Berechnung des Mindestauftragsbetrags (Subtotal + Tax - Discount) wird die Steuer berücksichtigt.<br/>**Nein** - Die Steuer ist bei der Berechnung des Mindestauftragsbetrags (Zwischensumme - Rabatt) nicht inbegriffen. |
...
| Schiff in die betreffenden Länder | Webseite | Identifiziert die Länder, in denen Sie kostenlosen Versand anbieten. Optionen: <br/>**Alle zugelassenen Länder** - Kunden aus jedem Land, das in der Store-Konfiguration angegeben ist, können kostenlosen Versand verwenden. <br/>**Bestimmte Länder** - Kunden aus nur bestimmten Ländern können kostenlosen Versand nutzen. |
...
| [!UICONTROL Calculate Handling Fee] | Webseite | Bestimmt, wie die Bearbeitungsgebühr berechnet wird (sofern vorhanden). Optionen: `Fixed` / `Percent` |
| [!UICONTROL Handling Fee] | Webseite | Die Höhe der Gebühren, die der Versandgebühr hinzugefügt werden, um die Kosten für die Bearbeitung der Sendung zu decken. Geben Sie den Wert als Dezimalzahl ein. Wenn die Gebühr beispielsweise auf einem Prozentsatz basiert, geben Sie 0,06 anstelle von 6 % ein. Geben Sie für einen festen Betrag `6.00`. |
...
| [!UICONTROL Ship to Applicable Countries] | Webseite | Identifiziert die Länder, in denen der Versand zum Tabellenpreis angeboten wird. Optionen: <br/>**`All Allowed Countries`**- Kunden aus allen in der Store-Konfiguration angegebenen Ländern können die Tabellentarifversandliste verwenden.<br/>**`Specific Countries`** - Kunden aus nur bestimmten Ländern können den Versand der Tabellenpreise nutzen. |
...
| Feld | [Anwendungsbereich](../../getting-started/websites-stores-views.md#scope-settings) | Beschreibung |
|--- |--- |--- |
| [!UICONTROL Enabled] | Webseite | Wenn diese Option aktiviert ist, kann die Bereitstellung im Store als Option im _Geschätzte Schifffahrt und Steuer_ im Warenkorb und im _Versand_ während des Checkouts. Optionen: `Yes` / `No` |
...
| [!UICONTROL UPS Type] | Store-Ansicht | Gibt die Methode an, mit der eine Verbindung zum UPS Versandsystem hergestellt wird. Optionen: <br/>**`United Parcel Service XML`**- (Standard) Ihr Store sendet eine XML-Datei mit Daten an UPS als Anfrage.<br/>**`United Parcel Service`** - Ihr Store sendet Schlüssel-Wert-Paare als Anfrage an UPS. <br/><br/>**_Hinweis:_**Der standardmäßige United Parcel Service-Typ ist für die Einstellung in Commerce geplant. Für neue Konfigurationen sollten Sie die [!UICONTROL United Parcel Service XML] Typ. |
...
| [!UICONTROL Gateway URL] | Webseite | Die URL, die eine Verbindung zum UPS-System herstellt, um dynamische Versandraten abzurufen. UPS stellt die Unterstützung von HTTP ein. Standardwert: `https://www.ups.com/using/services/rave/qcostcgi.cgi` |
| [!UICONTROL Title] | Store-Ansicht | Der Name, der für diese Versandmethode beim Checkout verwendet wird. |
| _[!UICONTROL UPS XML Account Settings]_ |  |  |
| [!UICONTROL Access License Number] | Webseite | Zugriffsnummer Ihres UPS-Versandkontos. |
| [!UICONTROL Gateway XML URL] | Webseite | Für den UPS XML-Dienst zeigt die folgenden URLs an, die zur Übertragung von XML-Daten erforderlich sind: Gateway XML URL, Tracking XML URL, Shipping Confirm XML URL, Shipping Accept XML URL |
...
| Benutzer-ID | Webseite | Ihre Benutzer-ID des UPS-Versandkontos. |
...
| [!UICONTROL Enable Negotiated Rates] | Webseite | (Nur UPS XML) Aktiviert/deaktiviert Sondertarife gemäß Ihrer Vereinbarung mit UPS. Optionen: `Yes` / `No` |
...
| [!UICONTROL Shipper Number] | Webseite | (Nur UPS XML) Die sechsstellige UPS Shipper Number ist erforderlich, damit auf ausgehandelte Raten verwiesen werden kann. |
| [!UICONTROL Container] | Webseite | Legt den Behältertyp fest, der für die Paketbestellungsversand verwendet wird. Optionen: `Customer Packaging` / `UPS Letter Envelope` / `Customer Packaging` / `UPS Letter Envelope` / `UPS Tube` / `UPS Express Box` / `UPS Worldwide 25 kilo` / `UPS Worldwide 10 kilo` |
| [!UICONTROL Weight Unit] | Webseite | Legt die Standardmaßeinheit für die Produktgewichtung in Ihrem Geschäft fest. Siehe [Dimensionsgewicht](../../stores-purchase/carriers.md#dimensional-weight) für weitere Informationen. |
| [!UICONTROL Tracking XML URL] | Webseite | (Nur UPS XML) Die UPS-URL, die zum Tracking von Paketen verwendet wird. |
| [!UICONTROL Destination Type] | Webseite | Legt den standardmäßigen Versandzieltyp fest. Optionen: `Business` / `Residential` |
| [!UICONTROL Maximum Package Weight] | Webseite | Legt die maximale Gewichtung fest, die ein Paket gemäß den UPS-Vorgaben erhalten kann. Wenn die bestellten Produkte die maximale Paketgewichtung überschreiten, ist diese Versandoption nicht verfügbar. Gemäß [UPS.com](https://www.ups.com/us/en/global.page), Packungen dürfen nicht mehr als 150 lbs (70 kg), fragen Sie bei Ihrem Versandunternehmen nach dem Höchstgewicht. |
...
| [!UICONTROL Calculate Handling Fee] | Webseite | Legt die Methode zur Berechnung der Bearbeitungsgebühr für den Versand von Tabellen fest. Optionen: <br>**`Fixed`**- Die Bearbeitungsgebühr ist ein Festpreis.<br>**`Percent`** - Die Bearbeitungsgebühr wird als Prozentsatz des Bestellbetrags erhoben. |
...
| [!UICONTROL Free Method] | Webseite | Gibt die Methode an, die für die kostenlose Versandmethode über UPS verwendet wird. Um den kostenlosen Versand zu deaktivieren, wählen Sie &quot;Keine&quot;. <br/><br/>**_Hinweis:_**Diese Methode ähnelt der Basismethode [Kostenloser Versand](../../stores-purchase/shipping-free.md), wird jedoch während des Checkout als UPS Versandoption angezeigt. |
...
| [!UICONTROL Ship to Applicable Countries] | Webseite | Gibt an, welches Land Kunden diese Versandmethode verwenden dürfen. Optionen: <br/>**`All Allowed Countries`**- Kunden von allen [Länder](../../getting-started/store-details.md#country-options) Diese Versandmethode kann in Ihrer Store-Konfiguration verwendet werden.<br/>**`Specific Countries`** - Nach Auswahl dieser Option wird die [!UICONTROL Ship to Specific Countries] angezeigt. Wählen Sie jedes Land in der Liste aus, in dem diese Versandmethode verwendet werden kann. |
| [!UICONTROL Show Method if Not Applicable] | Webseite | Bestimmt, ob UPS beim Checkout immer als Versandoption angezeigt wird. Optionen: <br/>**`Yes`**- UPS erscheint beim Checkout immer als Versandoption, auch wenn es nicht auf die Bestellung zutrifft.<br/>**`No`** - UPS erscheint beim Checkout nur dann als Versandoption, wenn es auf die Bestellung anwendbar ist. (Wenn beispielsweise die Bestellgewichtung die maximale Gewichtung überschreitet.) |
...
| [!UICONTROL Packages Request Type] | Webseite | Bestimmt, wie die Gewichtung für Sendungen mit mehreren Packages berechnet wird. Optionen: `Divide to equal weight (one request)` / `Use origin weight (multiple requests)` |
| [!UICONTROL Container] | Webseite | Legt den Behältertyp fest, der für die Paketbestellungsversand verwendet wird. Optionen: `Variable` / `Flat Rate Box` / `Flat Rate Envelope` / `Rectangular` / nicht rechteckig |
...
| [!UICONTROL Free Method] | Webseite | Legt die kostenlose Versandmethode über USPS fest oder kann durch Auswahl von `None`. <br/><br/>**_Hinweis:_**Diese Versandmethode ähnelt der kostenlosen Versandmethode Ihres Stores, ist jedoch als USPS-Versandoption aufgeführt und als USPS-Versand gekennzeichnet. |
...
| [!UICONTROL Ship to Applicable Countries] | Webseite | Gibt die Länder an, in denen Bestellungen versandt werden können. Optionen: <br/>**`All Allowed Countries`**- Kunden von allen [Länder](../../getting-started/store-details.md#country-options) Diese Versandmethode kann in Ihrer Store-Konfiguration verwendet werden.<br/>**`Specific Countries`** - Nach Auswahl dieser Option wird die [!UICONTROL Ship to Specific Countries] angezeigt. Wählen Sie jedes Land in der Liste aus, in dem diese Versandmethode verwendet werden kann. |
| [!UICONTROL Show Method if Not Applicable] | Webseite | Steuert die Anzeige des USPS-Versands beim Checkout. Optionen: <br/>**`Yes`**- USPS erscheint beim Checkout immer als Versandoption, auch wenn es nicht auf die Bestellung anwendbar ist.<br/>**`No`** - USPS erscheint beim Checkout nur dann als Versandoption, wenn es auf die Bestellung zutrifft (d. h. die Bestellgewichtung übersteigt die maximale Gewichtung). |
...
![FedEx-Kontoeinstellungen](./assets/delivery-methods-fedex-account-settings.png)<!-- zoom -->
...
| [!UICONTROL Meter Number] | Webseite | Ihre FedEx-Zählernummer. |
| [!UICONTROL Key] | Webseite | Ihr FedEx-Kontoschlüssel. |
| [!UICONTROL Password] | Webseite | Ihr FedEx-Kontokennwort. |
...
| [!UICONTROL Calculate Handling Fee] | Webseite | Bestimmt die Methode zur Berechnung der Bearbeitungsgebühren. Optionen: `Fixed Fee` / `Percentage` <br/><br/>**_Hinweis:_**Die Bearbeitungsgebühr ist optional und erscheint als zusätzliche Gebühr, die zu den Versandkosten von FedEx hinzugefügt wird. |
...
| [!UICONTROL Residential Delivery] | Webseite | Legen Sie je nachdem, ob Sie B2C (Business-to-Consumer) oder B2B (Business-to-Business) verkaufen, eine der folgenden Optionen fest: <br/>**`Yes`**- B2C-Sendungen<br/>**`No`** - B2B-Sendungen |
...
| [!UICONTROL Hub ID] | Webseite | Eine von FedEx bereitgestellte ID, die mit der Variablen [!DNL Smart Post] -Methode. |
| [!UICONTROL Free Method] | Webseite | Wählen Sie in der Liste die Versandmethode aus, die Sie für Angebote des kostenlosen Versands bevorzugen. <br/><br/>**_Hinweis:_**Diese Versandmethode ähnelt der normalen Free Shipping-Methode, ist jedoch in den FedEx-Versandoptionen aufgeführt und wird als FedEx-Versand identifiziert. |
...
| [!UICONTROL Ship to Applicable Countries] | Webseite | Gibt die Länder an, in denen Ihre Kunden nach FedEx versenden können. Optionen: <br/>**`All Allowed Countries`**- Kunden von allen [Länder](../../getting-started/store-details.md#country-options) Diese Versandmethode kann in Ihrer Store-Konfiguration verwendet werden.<br/>**`Specific Countries`** - Nach Auswahl dieser Option wird die [!UICONTROL Ship to Specific Countries] angezeigt. Wählen Sie jedes Land in der Liste aus, in dem diese Versandmethode verwendet werden kann. |
| [!UICONTROL Ship to Specific Countries] | Webseite | Gibt an, in welchen Ländern Ihre Kunden nach FedEx versenden können. |
| [!UICONTROL Debug] | Webseite | Bestimmt, ob das System zum Debugging ein Protokoll der Datenübertragungen zwischen Ihrem Store und FedEx verwaltet. Sofern kein Problem vorliegt, das verfolgt und protokolliert werden muss, sollte diese Option auf `No`. |
...
| [!UICONTROL Ready Time] | Webseite | Gibt an, wann das Paket in Stunden nach dem Absenden einer Bestellung zur Übernahme bereit ist. |
...
| [!UICONTROL Ship to Applicable Countries] | Webseite | Gibt an, welches Land Kunden diese Versandmethode verwenden dürfen. Optionen: <br/>**Alle zugelassenen Länder** - Alle Länder, für die der freie Versand erlaubt ist, können die kostenlose Versandmethode nutzen. Die zugelassenen Länder werden im Abschnitt [!UICONTROL General] Konfigurationsseite. <br/>**Bestimmte Länder** - Beschränkt diese Versandoption auf die Länder, die in der Liste &quot;Versand an bestimmte Länder&quot;angegeben sind. |
| [!UICONTROL Ship to Specific Countries] | Webseite | Gibt die Länder an, in denen DHL-Sendungen durchgeführt werden können. Diese Länderliste wird verwendet, wenn `Specific Countries` wird im Feld [!UICONTROL Ship to Applicable Countries] -Option. |
| [!UICONTROL Show Method if Not Applicable] | Webseite | Bestimmt, wann DHL beim Checkout als Versandmethode angezeigt wird. Optionen: <br/>**`Yes`**- DHL erscheint beim Checkout immer als Versandoption, auch wenn es nicht auf die Bestellung zutrifft.<br/>**`No`** - DHL erscheint beim Checkout nur dann als Versandoption, wenn es auf die Bestellung zutrifft (d. h. wenn die Bestellgewichtung die maximale Gewichtung überschreitet). |
